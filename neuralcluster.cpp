    ...
    for(int k = 0; k < steps; k++){
        ...
        for(int i = 0; i < weightsActive.size()-1; i++){
            ...
            for(int j = 0; j < weightsActive.size(); j++){
                signalReal += lastReal[j]*(weightsActive[i][j]+deltaMatrix[i][j]);
            }
    ...
    for(int k = 0; k < steps; k++){
        ...
        for(int i = 0; i < weightsActive.size()-1; i++){
            ...
            for(int j = 0; j < weightsActive.size(); j++){
                ...
                differenceSig += (lastReal[j]-beforelastReal[j])*(weightsActive[i][j]+deltaMatrix[i][j]);
            }
...
void NeuralCluster::learnEval(float learningSignal, float learningRate){

    float learn = minMax(learningSignal);
    //Overtrain temporary learned knowledge to fix weights
    for(int i = 0; i < weightsActive.size(); i++){
        for(int j = 0; j < weightsActive.size(); j++){
            weightsActive[i][j] += deltaMatrix[i][j]*learn*2.0;
            deltaMatrix[i][j] -= deltaMatrix[i][j]*(1.0-learn)*(1.0-learningRate);//deltaMatrix[i][j]*(learningRate)*(1.0/exp(1.0));
        }
    }
    ...
    for(int m = 0; m < weightsActive.size(); m++){
        ...
        //Normalize the inputs and outputs of each neuron so their absoulte sum is one (and zero unwanted wheigts)
        for(int j = 0; j < weightsActive.size(); j++){
            ...
            //if((i >= numInputs+numOutputs)&& (j >= numInputs+numOutputs) && (i < weightsActive.size())&& (j < weightsActive.size()-1) && mask[i][j]){ weightsActive[i][j] = 0.0; }
    ...
    for(int m = 0; m < weightsActive.size()-1; m++){
        ...
        for(int j = 0; j < weightsActive.size()-1; j++){
            ...
            float activationJ = (EnergyFlowReal[j]);
             ...
             meanOutput += (activationJ)*(weightsActive[j][i]+deltaMatrix[i][j]);



            balanceSignal += 2.0*(activationJ-0.5)*(weightsActive[i][j]+deltaMatrix[i][j]);


            if(weightsActive[i][j] < 0.0) meanInput += (activationJ)*(weightsActive[i][j]+deltaMatrix[i][j]);
            else meanInput += (1.0-activationJ)*(weightsActive[i][j]+deltaMatrix[i][j]);
            meanInputSignal +=  lastReal[j]*(weightsActive[i][j]+deltaMatrix[i][j]);

            meanOutputInactive += (weightsActive[j][i]+deltaMatrix[j][i]);
            meanInputEnergy +=  (lastReal[j])*abs(weightsActive[i][j]+deltaMatrix[i][j]);
            meanActivity += (activationJ);

            absDeviation += abs(activationJ-mean[j])*(weightsActive[i][j]+deltaMatrix[i][j]);
        ...
        for(int j = 0; j < weightsActive.size()-1; j++){
            ...
            float gradient = minMax((meanInputSignal-0.0*lastReal[i]*(impulseResponse[i]))/weightsActive.size())*minMax(-(meanInputSignal-0.0*lastReal[i]*(impulseResponse[i]))/weightsActive.size());

            if(type == synapseType[j][i]||type == synapseType[i][j] || synapseType[i][j] == -1 || synapseType[j][i] == -1) deltaMatrix[i][j] -= (1.0-activationJ)*activationI*exp(-balanceSignal/weightsActive.size())*(meanInputSignal/meanInputEnergy-0.0*lastReal[i]*(impulseResponse[i])/weightsActive.size())*learningRate*globalRMSError*globalRMSError;//*gradient*4.0;//*minMax(-8.0*((activationI)*(1.0-activationI)-(activationJ)*(1.0-activationJ)));//*minMax(16.0*relativeBehaviour[i][j]);

            if(type == synapseType[j][i]||type == synapseType[i][j] || synapseType[i][j] == -1 || synapseType[j][i] == -1) weightsActive[i][j] -= activationJ*activationI*((((0.0+(impulseResponse[j]+((impulseResponse[j]+impulseResponse[i])*((weightsActive[j][i])))))))/weightsActive.size())*learningRate;//*gradient*4.0;//*minMax(-8.0*((activationI)*(1.0-activationI)-(activationJ)*(1.0-activationJ)));//*minMax(16.0*relativeBehaviour[i][j]);

            if(type == synapseType[j][i]||type == synapseType[i][j] || synapseType[i][j] == -1 || synapseType[j][i] == -1) deltaMatrix[j][i] -= (activationI*(activationJ)*meanOutputInactive/weightsActive.size()-0.0*(1.0-activationJ)*(activationI)*(activationJ)*meanOutput/weightsActive.size())*learningRate*globalRMSError;            //if(type == synapseType[j][i]||type == synapseType[i][j] || synapseType[i][j] == -1 || synapseType[j][i] == -1) weightsActive[j][i] -= (activationI)*activationJ*(meanOutputInactive/weightsActive.size())*learningRate*globalRMSError;//*gradient*4.0;//*minMax(-8.0*((activationI)*(1.0-activationI)-(activationJ)*(1.0-activationJ)));//*minMax(16.0*relativeBehaviour[i][j]);
        }

        //float activationI = (EnergyFlowReal[i]);
        deltaMatrix[i][weightsActive.size()-1] -= (meanInputSignal+1.0*weightsActive[i][weightsActive.size()-1]-0.0*lastReal[i]*(impulseResponse[i]))/weightsActive.size()*learningRate;
    }
    ...
    float sumCounterClassyfier = 0;
        ...
        for(int i = 0; i < weightsActive.size()-1; i++){
            ...
            for(int j = 0; j < weightsActive[i].size(); j++){
                ...
                if(weightsActive[i][j] < 0.0) inPatternAnomaly += (activationJ)*abs(weightsActive[i][j]+deltaMatrix[i][j])*activationI;
                else inPatternAnomaly += (1.0-activationJ)*abs(weightsActive[i][j]+deltaMatrix[i][j])*activationI;

                if(weightsActive[i][j] < 0.0) inPatternAnomaly -= (1.0-activationJ)*abs(weightsActive[i][j]+deltaMatrix[i][j])*activationI;
                else inPatternAnomaly -= (activationJ)*abs(weightsActive[i][j]+deltaMatrix[i][j])*activationI;
...
*/
            ...
            meanChanging[i] = (256.0*meanChanging[i]+abs(lastReal[i]-EnergyFlowReal[i]))/(257.0);
